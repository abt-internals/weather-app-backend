...
async def forgot_password(user_mail: Email, db: AsyncSession = Depends(get_db)):
    return await forgot_pass(db, user_mail)


@router.post("/verifyotp")
async def verify_otp(user: OTP, db: AsyncSession = Depends(get_db)):
    return await verifyotp(db, user)


@router.post("/resendotp")
async def resend_otp(user_mail: Email, db: AsyncSession = Depends(get_db)):
    return await resend(db, user_mail)


@router.post("/resetpass")
async def reset_password(user_pass: empass, db: AsyncSession = Depends(get_db)):
